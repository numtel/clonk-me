...
export function Message({ item, chainId }) {
  return (
    <div className="msg">
      ...
      <span className="postdate">Posted on {new Date(item.createdAt.toString() * 1000).toLocaleString()}{item.lastChanged > 0n && (<em className="edited" title={new Date(item.lastChanged.toString() * 1000).toLocaleString()}>Edited</em>)}</span>
      ...
      {item.unsortedCount > 0n
        ...
        : (<button>0 Unsorted Replies</button>)}
    </div>
  );
}
